...
export const Avatar: React.FC<AvatarProps> = ({ 
  ...
  if (!user) {
    return (
      <div 
        className={`${sizes[size]} bg-gradient-to-br from-pink-500 via-purple-500 to-blue-500 rounded-full ${paddingSizes[size]} flex-shrink-0 ${onClick ? 'cursor-pointer hover:opacity-90 transition-opacity' : ''} ${className}`}
        onClick={onClick}
      >
        <div 
  ...
  if (user.avatarUrl) {
    return (
      <div 
        className={`${sizes[size]} flex-shrink-0 ${onClick ? 'cursor-pointer hover:opacity-90 transition-opacity' : ''} ${className}`}
        onClick={onClick}
      >
        <img
          src={user.avatarUrl}
