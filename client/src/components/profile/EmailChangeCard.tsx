...
export const EmailChangeCard: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data || response.message) {
        setSuccess('Verification email sent! Please check your new email address to confirm the change.');
  ...
  const hasPendingChange = user?.pendingEmail && user.pendingEmail !== user.email;
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="pt-2">
                <Button
                  type="submit"
                  loading={loading}
                  disabled={loading || !formData.newEmail.trim() || !formData.password.trim()}
                  className="w-full"
                >
