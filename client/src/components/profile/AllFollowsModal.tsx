...
export const AllFollowsModal: React.FC<AllFollowsModalProps> = ({ 
  ...
  const loadFollowData = async (tab: 'followers' | 'following', pageNum: number) => {
    try {
      setLoading(true);
      setError('');
      ...
      title={
        <div className="flex items-center space-x-4">
          <button
            ...
            style={{
              backgroundColor: activeTab === 'followers' 
      ...
      <div style={{ 
        ...
        <div className="space-y-4">
          ...
          <div className="text-center py-2">
            ...
            >
              {showLoadingState ? 'Loading...' : `${totalCount} ${activeTab === 'followers' ? 'Followers' : 'Following'}`}
            </p>
          </div>
            ...
            {showLoadingState ? (
              ...
              <div className="max-h-96 overflow-y-auto space-y-3">
                {currentData.map((item) => {
                  ...
                  return (
                    <div key={targetUser._id} className="flex items-center space-x-3 p-3 rounded-lg transition-colors duration-200"
                         style={{ backgroundColor: 'var(--color-secondary, #f1f5f9)' }}
                         onMouseEnter={(e) => {
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center space-x-2 mb-1">
                          ...
                          >
                            {getFullName(targetUser.firstName, targetUser.lastName)}
                          </h4>
                        </div>
                      ...
                      {!isCurrentUser && (
                        <div className="flex-shrink-0">
                          <button
                            onClick={() => handleFollow(targetUser._id, isFollowing)}
          ...
          {totalPages > 1 && !showLoadingState && !error && (
            <div className="flex items-center justify-between pt-4 border-t" 
              ...
              <button
                ...
                style={{
                  backgroundColor: page === 1 ? 'var(--color-muted, #f1f5f9)' : 'var(--color-primary, #3b82f6)',
                  color: page === 1 ? 'var(--color-muted-foreground, #64748b)' : 'white'
                }}
              >
                Previous
              </button>
              ...
              <button
                ...
                className="px-4 py-2 text-sm rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                style={{
                  backgroundColor: !hasNext ? 'var(--color-muted, #f1f5f9)' : 'var(--color-primary, #3b82f6)',
                  color: !hasNext ? 'var(--color-muted-foreground, #64748b)' : 'white'
                }}
              >
                Next
              </button>
